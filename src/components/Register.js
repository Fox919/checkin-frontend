...
// 多語言字典
const t = {
  'zh-TW': { title: "活動人員登記", name: "姓名", phone: "電話", email: "電子郵件 (選填)", type: "身分", submit: "提交登記並生成碼", success: "登記成功！請截圖保存下方的二維碼。", guest: "來賓", volunteer: "義工 (規劃中)", student: "學員 (規劃中)", retry: "重新登記", error: "失敗" },
  'zh-CN': { title: "活动人员登记", name: "姓名", phone: "电话", email: "电子邮件 (选填)", type: "身分", submit: "提交登记并生成码", success: "登记成功！请截图保存下方的二维码。", guest: "来宾", volunteer: "义工 (规划中)", student: "学员 (规划中)", retry: "重新登记", error: "失败" },
  'en-US': { title: "Registration", name: "Name", phone: "Phone", email: "Email (Optional)", type: "Role", submit: "Submit & Generate QR", success: "Registration successful! Please save the QR code below.", guest: "Guest", volunteer: "Volunteer (Planned)", student: "Student (Planned)", retry: "Register Again", error: "Error" }
};
...
const Register = () => {
  ...
  const handleChange = (e) => {
    ...
    // 改進 1: 電話欄位只允許數字
    if (name === 'phone') {
      const isNumber = /^\d*$/.test(value); // 檢查是否為數字
      if (isNumber) {
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '400px', margin: 'auto', textAlign: 'center', fontFamily: 'sans-serif' }}>
      ...
      <h2 style={{ color: '#333' }}>{translations.title}</h2>
